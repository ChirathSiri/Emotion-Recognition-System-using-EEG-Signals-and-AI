...
if __name__ == '__main__':
    ...
    if preprocessed_raw_data is not None:
        ...
        if data_for_feature is not None:
            # Splitting data into train and test sets
            X_train, X_test, y_train, y_test = train_test_split(eeg_band_data, labels_for_classification, test_size=0.3,
                                                                random_state=42)

            # Building the CNN model
            model = Sequential()
            model.add(Conv1D(filters=32, kernel_size=3, activation='relu', input_shape=(40, 8064)))
